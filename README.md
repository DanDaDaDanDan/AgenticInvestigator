...
3. **BOOTSTRAP**: Initial research, capture sources, draft findings
4. **QUESTION**: Apply 35 analytical frameworks + custom questions
5. **FOLLOW**: Pursue all leads → reconcile with findings → check curiosity
6. **WRITE**: Generate publication articles (short/medium/full + PDFs)
7. **VERIFY**: Check all 11 gates before completion
...
| 3 | Reconciliation | Lead results sync with findings, stale findings updated |
| 4 | Article | Publication-ready article exists |
| 5 | Sources | All citations verified (semantic + computational) |
| 6 | Integrity | Journalistic standards met (multi-agent debate) |
| 7 | Legal | Legal risk assessment passed (multi-agent debate) |
...
| File | Purpose |
|------|---------|
| `findings/` | Decomposed findings with independent lifecycle |
...
| `/reconcile` | Sync lead results with findings | `context: fork` |
| `/curiosity` | Check if leads exhausted | `context: fork` |
| `/capture-source` | Capture web evidence | - |
| `/article` | Generate articles | `context: fork` |
| `/verify` | Check 11 gates | `context: fork` |
| `/balance-audit` | Check stakeholder balance (Gate 8) | `context: fork` |
| `/completeness-audit` | Check coverage completeness (Gate 9) | `context: fork` |
| `/significance-audit` | Check investigation significance (Gate 10) | `context: fork` |
