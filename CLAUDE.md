...
- Do NOT create non-canonical or duplicate findings files (must be `findings/F###.md` with matching `id: F###`; run `scripts/audit-findings.js` before WRITE)
...
- Do NOT use Write tool to create evidence files - ALWAYS use osint_get + osint-save.js workflow
- Do NOT create "synthesis" or "compilation" sources - One Source = One URL (fetched, not written)
- If synthesis is needed for clarity, write it as an `analysis/` memo that cites underlying `S###` sources with direct quotes; never treat synthesis as a source
- Do NOT invent capture signatures - they must be generated by osint-save.js
- Do NOT create evidence folders manually - all evidence must come from actual URL captures
